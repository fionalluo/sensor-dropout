#!/usr/bin/env python3
"""
Training script for a single SB3 PPO subset policy.

This script trains a single SB3 PPO policy for a specific observation subset
(e.g., env1, env2, env3, env4) and saves it to the policies directory.

This is a parallelizable version of train_subset_policies.py that handles
one subset at a time, enabling cluster-level parallelization.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train SB3 PPO policy for a single eval subset')
    ...
    parser.add_argument('--subset_name', type=str, required=True,
                       help='Name of the subset to train (e.g., env1, env2, env3, env4)')
...
def main():
    """Main entry point for training a single subset policy."""
    args = parse_args()
    ...
    print(f"Training {args.policy_type.upper()} policy for task: {config.task}")
    print(f"Output directory: {output_dir}")
    print(f"Subset: {args.subset_name}")
    
    # Get eval keys for this specific subset
    if hasattr(config, 'eval_keys') and hasattr(config.eval_keys, args.subset_name):
        env_keys = getattr(config.eval_keys, args.subset_name)
        eval_keys = {
            'mlp_keys': env_keys.mlp_keys,
            'cnn_keys': env_keys.cnn_keys
        }
    else:
        print(f"Warning: No eval_keys.{args.subset_name} found, using default patterns")
        eval_keys = {'mlp_keys': '.*', 'cnn_keys': '.*'}
    
    # Train policy for this specific subset
    policy_path = train_subset_policy(
        config, args.subset_name, eval_keys, output_dir, device, 
    ...
    print(f"\nTrained policy for {args.subset_name}:")
