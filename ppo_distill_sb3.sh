...
# Base log directory
BASE_LOGDIR=~/logdir/baselines/ppo_distill_sb3
...
echo "SB3 Distillation Configuration:"
...
for CONFIG in "${CONFIGS[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "Running PPO Distill SB3 baseline with config ${CONFIG} and seed ${SEED}"
    echo "Expert policy directory: ${EXPERT_POLICY_DIR}"
    echo "Student policy type: ${STUDENT_POLICY_TYPE}"
    echo "Logging to: ${LOGDIR}"

    timeout 8h python3 -u baselines/ppo_distill_sb3/train.py \
      --configs ${CONFIG} \
...
echo "All PPO Distill SB3 tasks complete." 