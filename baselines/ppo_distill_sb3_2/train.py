#!/usr/bin/env python3
"""
Training script for multi-teacher to single student SB3 distillation.
Trains a student policy that learns from multiple teacher policies using pure distillation.
"""
...
from baselines.ppo_distill_sb3_2.multi_teacher_distill import train_multi_teacher_distillation
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train multi-teacher distillation agent")
...
def load_config(configs_names=None, policy_type='ppo_distill_sb3_2'):
...
def main():
    ...
    # Load configuration
    config = load_config(args.configs, 'ppo_distill_sb3_2')
    ...
    total_episodes = config.total_timesteps // config.steps_per_rollout
    
    print(f"Training Multi-Teacher Distillation agent...")
    print(f"Task: {config.task}")
    print(f"Total timesteps: {config.total_timesteps}")
    print(f"Steps per rollout: {config.steps_per_rollout}")
    print(f"Number of environments: {config.num_envs}")
    print(f"Number of teacher configs: {config.num_eval_configs}")
    print(f"Estimated episodes: {total_episodes}")
    
    # Train the agent
    trained_agent = train_multi_teacher_distillation(
        config=config,
    ...
    if hasattr(config, 'save_model') and config.save_model:
        save_path = f"ppo_distill_sb3_2_{config.task}_{config.seed}.zip"
