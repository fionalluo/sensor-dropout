...
def train_full_eval(
  ...
  if args.teacher_anneal:
    should_full_rollout = embodied.when.EveryAnneal(args.teacher_anneal_start_freq, args.teacher_anneal_end_freq, args.teacher_anneal_start, args.teacher_anneal_end, args.steps)
  else:
    should_full_rollout = embodied.when.Every(args.full_policy_rollout_every)
  
  # Privileged dropout condition
  dropout_anneal = embodied.when.Anneal(args.dropout_start_freq, args.dropout_end_freq, args.dropout_start, args.dropout_end, args.steps)
  ...
  if args.student_only_buffer:
    print("BUFFER: Setting teacher to be excluded from train replay buffer")
    driver_train.on_step_conditional(train_replay.add)  # add to train replay only if mode is not teacher 'full_train_no_replay'
  else:
  ...
  if not args.priv_dropout: # default, no dropout
    full_policy_train = lambda *args: agent.policy(
      *args, mode='explore' if should_expl(step) else 'full_train')
  else:
    full_policy_train = lambda *args: agent.policy(
        *args, mode='explore' if should_expl(step) else 'full_train', dropout=dropout_anneal.get_prob())
  ...
  while step < args.steps:
    if args.priv_dropout:
      dropout_anneal.step()  # step forward the dropout_anneal variable
    ...
    if should_full_rollout(algo_updates):
      if args.student_only_buffer:
        # print("BUFFER: Rolling out teacher, but not adding to replay buffer")
        driver_train(full_policy_train, episodes=1, mode='skip_on_step_conditional')  # this mode indicates, don't run conditional on_step
      else:
        # print("BUFFER: Rolling out teacher, and adding to replay buffer")
