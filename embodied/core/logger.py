...
class WandBOutput:
  ...
  def __init__(self, pattern, logdir, config):
    ...
    import wandb
    wandb.init(
        project="dv3_distill_gridblindpick7_reward",
        name=logdir.name,
        entity='fionalluo',
        config=dict(config),
        resume="allow",
  ...
  def __call__(self, summaries):
    ...
    wandb = self._wandb
    for step, name, value in summaries:
      if "_heatmap" in name and len(value.shape) == 2:
        # Special post-processing for logging penalty heatmaps
        print("Logging heatmap with text")
        from embodied.core.visuals import add_text_labels
        # print("VALUE", value)
        # print("VALUE TYPE", type(value))
        # print("VALUE SHAPE", value.shape)
        bystep[step][name] = wandb.Image(add_text_labels(value, name))
        # print("HEATMAP\n", bystep[step][name])

      elif len(value.shape) == 0 and self._pattern.search(name):
        bystep[step][name] = float(value)
      elif len(value.shape) == 1:
        ...
        print("NAME, VALUE", name, value, value.shape)
        print("VALUE -1", value.shape[-1])
        print("VALUE MAX", np.max(value))
        # Check if the last dimension is 3 (RGB image) and the maximum value is greater than 1
        if value.shape[-1] == 3 and np.max(value) > 1:
          bystep[step][name] = wandb.Image(value)  # Directly pass the image without modification
        else:
          print("NAME, VALUE", name, value, value.shape)
          value = np.clip(255 * value, 0, 255).astype(np.uint8)
          value = np.transpose(value, [2, 0, 1])
