...
# Base log directory
BASE_LOGDIR=~/logdir/baselines/ppo_distill_sb3_2
...
echo "Multi-Teacher SB3 Distillation Configuration:"
echo "  Expert policies: ${EXPERT_POLICY_TYPE} (from ${BASE_POLICY_DIR})"
echo "  Student agent: ${STUDENT_POLICY_TYPE}"
echo "  Multi-teacher approach: Each episode cycles through different teachers"
...
for CONFIG in "${CONFIGS[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "Running Multi-Teacher PPO Distill SB3 baseline with config ${CONFIG} and seed ${SEED}"
...
echo "All Multi-Teacher PPO Distill SB3 tasks complete." 