...
# Base log directory
BASE_LOGDIR=~/logdir/baselines/ppo_lstm
...
for CONFIG in "${CONFIGS[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    ...
    echo "Running PPO LSTM baseline with config ${CONFIG} and seed ${SEED}, logging to ${LOGDIR}"

    timeout 4h python3 -u baselines/ppo_lstm/train.py \
      --configs ${CONFIG} \
      --seed "$SEED"
