...
parser.add_argument("--num_eval_episodes", type=int, default=10, help="Number of episodes to evaluate")
...
def rollout_policy(env, agent, num_eval_episodes, rl_device):
    ...
    while total_episodes < num_eval_episodes:
        # Use inference_mode if possible, else fallback to no_grad
        try:
...
def main():
    ...
    agent.reset()
    total_rewards, episode_lengths = rollout_policy(env, agent, args_cli.num_eval_episodes, rl_device)
    ...
    print(f"[RESULT] Checkpoint: {os.path.basename(args_cli.checkpoint)}")
