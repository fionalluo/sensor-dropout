...
for ((config_idx=0; config_idx<$NUM_CONFIGS; config_idx++)); do
  ...
  for ((seed_idx=0; seed_idx<$NUM_SEEDS; seed_idx++)); do
    ...
    HELPER_SCRIPT="ppo_dropout_job_${config_idx}_${seed_idx}.sh"

    cat << EOF > $HELPER_SCRIPT
#!/usr/bin/env bash
## dj-partition settings
#SBATCH --job-name=ppo_dropout
#SBATCH --output=outputs/ppo_dropout_%A_%a.out
#SBATCH --error=outputs/ppo_dropout_%A_%a.err
#SBATCH --ntasks=1
...
timeout 12h python3 -u baselines/ppo_dropout/train.py \\
  --configs "\$CONFIG" \\
  --seed "\$SEED"
...
echo "All PPO Dropout jobs submitted." 